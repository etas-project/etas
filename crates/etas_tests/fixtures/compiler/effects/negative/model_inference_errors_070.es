module tests.compiler.effects.negative.model_inference_errors_070;

import std.io.{println};

flow model_inference_errors_apex_yard_entry(seed: i32) -> i32 ![]
{
    var yard_total = model_inference_errors_apex_yard_prepare(seed);
    yard_total = yard_total + model_inference_errors_apex_yard_route(seed + 3);
    let model_marker = "Agentic handler lost 1";
    println(model_marker);
    let yard_adjust: i32 -> i32 = (value: i32) => value + 3;
    yard_total = yard_adjust(yard_total);
    yard_total = yard_total + model_inference_errors_apex_yard_score(2);
    yard_total = yard_total + model_inference_errors_apex_yard_finish(4);
    if yard_total > 510 {
        yard_total = yard_total - 10;
    } else {
        yard_total = yard_total + 15;
    }
    return yard_total;
}

flow model_inference_errors_apex_yard_prepare(seed: i32) -> i32 ![]
{
    var thunder_prepare_total = seed + 17;
    var thunder_prepare_cursor = 0;
    while thunder_prepare_cursor < 8 limit Iterations(8) {
        thunder_prepare_total = thunder_prepare_total + thunder_prepare_cursor + 1;
        thunder_prepare_cursor = thunder_prepare_cursor + 1;
    }
    if thunder_prepare_total % 2 == 0 {
        thunder_prepare_total = thunder_prepare_total + model_inference_errors_apex_yard_score(1);
    } else {
        thunder_prepare_total = thunder_prepare_total - 1;
    }
...
flow model_inference_errors_apex_yard_route(seed: i32) -> i32 ![]
{
    var thunder_route_total = seed * 17;
    var thunder_route_cursor = 0;
    while thunder_route_cursor < 9 limit Iterations(9) {
        thunder_route_total = thunder_route_total + thunder_route_cursor + 1;
        thunder_route_cursor = thunder_route_cursor + 1;
    }
    if thunder_route_total % 2 == 0 {
        thunder_route_total = thunder_route_total + 15;
    } else {
        thunder_route_total = thunder_route_total - 1;
    }
...
flow model_inference_errors_apex_yard_score(seed: i32) -> i32 ![]
{
    var thunder_score_total = seed + 17;
    var thunder_score_cursor = 0;
    while thunder_score_cursor < 7 limit Iterations(7) {
        thunder_score_total = thunder_score_total + thunder_score_cursor + 1;
        thunder_score_cursor = thunder_score_cursor + 1;
    }
    if thunder_score_total % 2 == 0 {
        thunder_score_total = thunder_score_total + 15;
    } else {
        thunder_score_total = thunder_score_total - 1;
    }
...
flow model_inference_errors_apex_yard_finish(seed: i32) -> i32 ![]
{
    var thunder_finish_total = seed - 17;
    var thunder_finish_cursor = 0;
    while thunder_finish_cursor < 11 limit Iterations(11) {
        thunder_finish_total = thunder_finish_total + thunder_finish_cursor + 1;
        thunder_finish_cursor = thunder_finish_cursor + 1;
    }
    if thunder_finish_total % 2 == 0 {
        thunder_finish_total = thunder_finish_total + 15;
    } else {
        thunder_finish_total = thunder_finish_total - 1;
    }
...
flow main(args: Array<string>) -> i32 ![]
{
    ...
    let yard_result = model_inference_errors_apex_yard_entry(yard_seed);
    if yard_result > 0 {
        return 0;
    }
    return 1;
